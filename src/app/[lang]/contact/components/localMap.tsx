...
export default function LocalMap({ className, map }: Readonly<{ className?: string; map: Map }>) {
	return (
		<div className={`${className} lg:bg-primary absolute left-1/2 z-1 h-[75%] w-full max-w-[450px] -translate-x-1/2 lg:bottom-14 lg:left-full lg:h-71 lg:w-137.5 lg:max-w-[650px] lg:-translate-x-0`}>
			<div className={`text-quinary absolute top-1/2 left-12 hidden h-40 -translate-y-1/2 items-center text-center text-4xl uppercase lg:flex`}>
				<p>{map.heading}</p>
			</div>
			<div className={`absolute top-1/2 w-full max-w-md -translate-y-1/2 p-4 lg:-right-30 lg:p-0`}>
				<p className={"text-quinary mb-6 text-center text-3xl uppercase lg:hidden"}>- {map.heading} -</p>
